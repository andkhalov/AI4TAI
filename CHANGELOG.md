...
- Репозиторий содержит только инструмент. Учебные материалы (документы
  для RAG, раздатка, выгрузки для практики) вынесены из репозитория и
  выдаются слушателям отдельным комплектом.
- `data/catalog_mock.json` → `src/fixtures/catalog_mock.json`: это
  фикстура кода для режима `mock`, а не учебные данные.
- Учебная база собирается в `.data/demo.sqlite` (в `.gitignore`).
- Тесты и CI используют собственные фикстуры `tests/fixtures/docs`.
